...
export default function SplashScreen({ onFinish }) {
  ...
  useEffect(() => {
    ringScale1.value = withDelay(200, withTiming(1, { duration: 700, easing: Easing.out(Easing.cubic) }));
    ringOpacity1.value = withDelay(200, withTiming(0.12, { duration: 400 }));

    ringScale2.value = withDelay(400, withTiming(1, { duration: 800, easing: Easing.out(Easing.cubic) }));
    ringOpacity2.value = withDelay(
      400,
      withSequence(withTiming(0.1, { duration: 400 }), withDelay(500, withTiming(0, { duration: 500 })))
    );

    ringScale3.value = withDelay(550, withTiming(1, { duration: 900, easing: Easing.out(Easing.cubic) }));
    ...
    wordmarkY.value = withDelay(700, withTiming(0, { duration: 500, easing: Easing.out(Easing.cubic) }));

    taglineOpacity.value = withDelay(950, withTiming(1, { duration: 500 }));
    taglineY.value = withDelay(950, withTiming(0, { duration: 500, easing: Easing.out(Easing.cubic) }));
  ...
  return (
    <Animated.View style={[splashStyles.root, screenStyle]}>
      <LinearGradient colors={[C.primary + "18", "transparent"]} style={splashStyles.glow} />

      <Animated.View style={[splashStyles.ring, { width: 320, height: 320, borderRadius: 160 }, ring3Style]} />
      <Animated.View style={[splashStyles.ring, { width: 230, height: 230, borderRadius: 115 }, ring2Style]} />
      <Animated.View style={[splashStyles.ring, { width: 160, height: 160, borderRadius: 80 }, ring1Style]} />

      <Animated.View style={[splashStyles.logoWrap, logoStyle]}>
        <LinearGradient colors={[C.primary, "#C9614A"]} style={splashStyles.logoGradient} start={{ x: 0.1, y: 0.1 }} end={{ x: 0.9, y: 0.9 }}>
      ...
      <Animated.Text style={[splashStyles.tagline, taglineStyle]}>Reverse what can be reversed.</Animated.Text>

      <View style={splashStyles.bottomBar}>
        <View style={[splashStyles.bottomLine, { backgroundColor: C.primary }]} />
      </View>
    </Animated.View>
  );
}
...
const splashStyles = StyleSheet.create({
  ...
  glow: { position: "absolute", width: 400, height: 400, borderRadius: 200, top: H / 2 - 200, left: W / 2 - 200 },
  ring: { position: "absolute", borderWidth: 1, borderColor: C.primary },
  logoWrap: { marginBottom: 28 },
  logoGradient: { width: 80, height: 80, borderRadius: 26, alignItems: "center", justifyContent: "center" },
  logoMark: { width: 44, height: 44, alignItems: "center", justifyContent: "center", position: "relative" },
  arc: { position: "absolute", width: 30, height: 30, borderRadius: 15, borderWidth: 3, borderColor: "rgba(255,255,255,0.9)" },
  arcTop: { top: 0, left: 0, borderBottomColor: "transparent", borderRightColor: "transparent" },
  arcBottom: { bottom: 0, right: 0, borderTopColor: "transparent", borderLeftColor: "transparent" },
  logoDot: { width: 7, height: 7, borderRadius: 3.5, backgroundColor: "#FFFFFF" },
  textGroup: { alignItems: "center" },
  wordmark: { fontSize: 36, fontFamily: "PlusJakartaSans_700Bold", letterSpacing: -1 },
  tagline: { fontSize: 14, color: C.mutedFg, fontFamily: "DMSans_400Regular", letterSpacing: 0.3, marginTop: 10 },
