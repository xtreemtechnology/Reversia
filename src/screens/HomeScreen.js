...
const getHealthSummary = (glucoseValues) => {
  if (!glucoseValues.length) {
    return {
      score: null,
      label: "No data",
      inRangeText: "Add a glucose reading",
    };
  }
  ...
  const score = Math.max(
    ...
    Math.min(
      ...
      Math.round(
        inRangePercent * 0.7 +
          (average >= 70 && average <= 140 ? 30 : average <= 180 ? 18 : 10)
      )
    )
  );
...
const getConsecutiveLogDays = (logs) => {
  const uniqueDays = new Set(
    logs.map((log) => getDateKey(log.timestamp)).filter(Boolean)
  );
...
const getCalorieTarget = (userData = {}) => {
  ...
  if (level.includes("very active")) {
    return 2900;
  }
  if (level.includes("moderately active")) {
    return 2600;
  }
  if (level.includes("lightly active")) {
    return 2350;
  }
...
const getProfileFocus = (userData = {}) => {
  const condition =
    userData.diabetesType || userData.healthStatus || "Profile incomplete";
  ...
  let message =
    "We will tailor reminders and insights using your saved profile.";
...
const getDailyPlanItems = (userData = {}, hasGlucoseData = false) => {
  ...
  if (activity.includes("sedentary")) {
    ...
    activity.includes("moderately active") ||
...
const TopSwitcher = ({
  ...
  useEffect(() => {
    ...
    const currentIndex = CATEGORIES.findIndex(
      (category) => category.key === displayCategory
    );
      ...
      >
        ...
        {trendPath ? (
          <Svg width={chartWidth * 0.48} height={chartHeight * 0.8}>
            ...
            <Path
              d={`${trendPath} L${chartWidth * 0.48},${chartHeight * 0.8} L0,${
                chartHeight * 0.8
              } Z`}
              fill="url(#fade)"
            />
      ...
      {trendPath ? (
        <Svg width={chartWidth * 0.8} height={chartHeight}>
          ...
          <Path
            d={`${trendPath} L${
              chartWidth * 0.8
            },${chartHeight} L0,${chartHeight} Z`}
            fill="url(#glucoseFade)"
          />
        </Svg>
      ) : (
        <View style={styles.emptyTrendCard}>
  ...
  const renderSleep = () => (
    ...
    >
      ...
      {lastSleepLog &&
      (lastSleepLog.lightPercent || lastSleepLog.remPercent) ? (
        <View style={styles.stagesBar}>
          <View
            style={[
              styles.stage,
              {
                flex: lastSleepLog.lightPercent || 8,
        ...
        <View style={styles.stagesBar}>
          <View
            style={[styles.stage, { flex: 1, backgroundColor: "#E5E7EB" }]}
          />
        </View>
      )}
      ...
      <View style={styles.sleepStatsRow}>
        ...
        <View style={styles.sleepStat}>
          <Text style={styles.sleepStatValue}>
            {lastSleepLog?.efficiency || sleepEfficiency || "--"}%
          </Text>
        ...
        <View style={styles.sleepStat}>
          <Text
            style={[
              ...
              { color: lastSleepLog ? "#10B981" : "#9CA3AF" },
            ]}
          >
    ...
    <View style={styles.activityCard}>
      ...
      <View style={styles.activityStatsRow}>
        ...
        <View style={styles.activityStat}>
          <Text style={styles.activityStatValue}>
            {exerciseLogs.reduce(
              (sum, log) => sum + (Number(log.value) || 0),
              0
            ) || "0"}
          </Text>
  ...
  const renderBMI = () => (
    <View style={styles.bmiCard}>
      ...
      <View style={styles.bmiStatsRow}>
        ...
        <View style={styles.bmiDivider} />
        <View style={styles.bmiStat}>
          <Text style={[styles.bmiStatValue, { fontSize: 14 }]}>
          ...
          >
            <View
              style={[
                ...
                isCompactScreen && styles.nutritionHeaderCompact,
              ]}
            >
              <View>
              ...
              <TouchableOpacity
                style={[
                  styles.logFoodBtn,
                  isCompactScreen && styles.logFoodBtnCompact,
                ]}
            ...
            <View style={{ marginTop: 14 }}>
              <MacroBar
                ...
                percent={Math.min(
                  100,
                  Math.round((mealTotals.protein / proteinTarget) * 100) || 0
                )}
                color="#10B981"
              />
              <MacroBar
                ...
                percent={Math.min(
                  100,
                  Math.round((mealTotals.carbs / carbTarget) * 100) || 0
                )}
                color="#F59E0B"
              />
              <MacroBar
                ...
                percent={Math.min(
                  100,
                  Math.round((mealTotals.fats / fatTarget) * 100) || 0
                )}
                color="#3B82F6"
              />
            </View>
  ...
  return (
    <Animated.View
      style={{
        transform: [{ translateX: dragX }, { scale }, { rotate }],
        opacity: dragOpacity,
      }}
      ...
      <View style={styles.topSwitchStage}>
        {previousCategory && previousCategory !== displayCategory && (
          ...
          >
            <Animated.View
              pointerEvents="none"
              style={[styles.topSwitchDim, { opacity: outgoingDimOpacity }]}
            />
    ...
    <View style={dateStyles.row}>
      ...
      <View style={dateStyles.pill}>
        <Ionicons
          ...
          style={{ marginRight: 6 }}
        />
        <Text style={dateStyles.text}>
          Today, {dayNum} {dayName}
        </Text>
      </View>
...
export default function HomeScreen({ navigation }) {
  ...
  const healthSummary = useMemo(
    () => getHealthSummary(glucoseValues),
    [glucoseValues]
  );
  ...
  const todayLogs = useMemo(() => {
    ...
    return logs.filter(
      (log) =>
        log?.timestamp && getDateKey(log.timestamp) === getDateKey(new Date())
    );
  }, [logs]);
  ...
  const sleepStatus =
    lastSleepLog?.status || (lastSleepLog?.value ? "Good" : "No data");
  ...
  const profileFocus = useMemo(
    () => getProfileFocus(userData || {}),
    [userData]
  );
  const calorieTarget = useMemo(
    () => getCalorieTarget(userData || {}),
    [userData]
  );
      ...
      <AnimatedScreen style={{ flex: 1 }}>
        ...
        >
          ...
          <View
            style={[
              styles.mainActions,
              isCompactScreen && styles.mainActionsCompact,
            ]}
          >
            <TouchableOpacity
          ...
          <View style={styles.profileSummaryCard}>
            ...
            <View style={styles.profileChipsRow}>
              {profileFocus.chips.length ? (
                ...
                <Text style={styles.profileSummaryMuted}>
                  Complete setup to unlock personalized guidance.
                </Text>
              )}
            </View>
          </View>
          ...
          <View style={styles.progressCard}>
            <View>
              ...
              <Text style={styles.progressSub}>
                {todayLogs.length} log{todayLogs.length === 1 ? "" : "s"} saved
                today
              </Text>
            </View>
          ...
          {activeCategory === "overview" && (
            <View>
              ...
              <View
                style={[
                  styles.grid,
                  isCompactScreen && styles.gridCompact,
