...
export default function AccountSetupGender({ navigation }) {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      Alert.alert("Connection Error", "We couldn't save your selection. Please try again.");
    } finally {
      setLoading(false);
    }
  };
...
const styles = StyleSheet.create({
  ...
  activeCardFemale: { backgroundColor: '#E0E7FF' }, // Slightly softer purple/blue tint
  avatar: { width: '90%', height: '90%' },
  genderLabel: { fontSize: 18, fontWeight: '600', color: '#9CA3AF' },
  activeLabelMale: { color: '#2563EB' },
  activeLabelFemale: { color: '#825CFF' }, // Matching brand color
